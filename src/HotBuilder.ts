...
export class HotBuilder
{
		...
		})
	{
		...
		if (deps != null)
		{
			...
			if (deps.webExport != null)
			{
				...
				if (assetsToCopyObj != null)
				{
					...
					{
						await copyFilesFromAssets (assetsToCopyObj.components, 
							`${outDir}/public/components/`, `${baseUrl}/public/components/`, files.components);
					}
				}
			}
		}
		...
		let componentsFilesStr: string = getFileStr (files.components, true);
...
`// Auto-generated by HotStaq on ${new Date ().toString ()}
